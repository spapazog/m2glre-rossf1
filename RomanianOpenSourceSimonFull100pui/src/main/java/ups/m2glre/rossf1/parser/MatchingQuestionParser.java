...
public class MatchingQuestionParser extends QuestionParser  {
    ...
    private void parseSubQuestion(Element subQuestionElement) 
        ...
        if (textElement == null)
            throw new Throwable(MoodleXML.TAG_SUBQUESTION + " node doesn't contain" 
                    + MoodleXML.TAG_TEXT + "element.");
        
        Element answerElement = subQuestionElement.getChild(MoodleXML.TAG_SUBQUESTION_ANSWER);
        if (answerElement == null)
            throw new Throwable(MoodleXML.TAG_SUBQUESTION + " node doesn't contain"
                    + MoodleXML.TAG_SUBQUESTION_ANSWER + "element.");
